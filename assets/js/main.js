...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function nonOverlappingPos(existingPositions) {
    ...
    do {
      pos = randomPos();
      attempts++;
      const tooClose = existingPositions.some(function (p) {
        const dx = p.x - pos.x;
        const dy = p.y - pos.y;
  ...
  function spawnClickLabel(x, y) {
    ...
    setTimeout(function () { label.remove(); }, 520);
  }
  ...
  function buildHeads() {
    ...
    for (let i = 0; i < TOTAL_HEADS; i++) {
      ...
      head.setAttribute('data-emoji', MASCOT_EMOJI);
  ...
  function onHeadClick(e) {
    ...
    spawnClickLabel(rect.left + HEAD_SIZE / 2, rect.top - arenaTop);
